# Articles

## Computer Science

[Awesome Cheatsheets](https://lecoupa.github.io/awesome-cheatsheets/)

[QuickRef.ME - Quick Reference Cheat Sheet](https://quickref.me/)

[Recursion Explained with the Flood Fill Algorithm (and Zombies and Cats) - The Invent with Python Blog](http://inventwithpython.com/blog/2011/08/11/recursion-explained-with-the-flood-fill-algorithm-and-zombies-and-cats/)

[Free for Developers](https://free-for.dev/#/)

[jlevy/the-art-of-command-line: Master the command line, in one page](https://github.com/jlevy/the-art-of-command-line?tab=readme-ov-file#meta)

[Programming Praxis | A collection of etudes, updated weekly, for the education and enjoyment of the savvy programmer](https://programmingpraxis.com/)

[Best Developer Websites: Programming News, Tutorials & More](https://stackify.com/18-websites-every-developer-should-visit-right-now/)

[Developer Roadmaps - roadmap.sh](https://roadmap.sh/)

[Teach Yourself Programming in Ten Years](http://norvig.com/21-days.html)

[25 Must-Visit Killer Websites For Developers | Mr. Ånand](https://mranand.com/blogs/25-must-visit-killer-websites-for-developers/)

[How to Change Your Application Icon from the Terminal in Linux](https://www.maketecheasier.com/change-application-icon-from-terminal-linux/)

[How to Ask Technical Questions | The Odin Project](https://www.theodinproject.com/guides/community/how_to_ask)

[Programming Wiki](https://www.reddit.com/r/learnprogramming/wiki/faq/#wiki_getting_started)

[Introduction](http://alumni.cs.ucr.edu/~pdiloren/C++_Pointers/)

[List.community](https://list.community/)

[Teach Yourself Computer Science](https://teachyourselfcs.com/)

[Realtime Colors](https://realtimecolors.com/?colors=000000-ffffff-8fb3ff-ebf1ff-d41d6d)

[SVG Repo - Free SVG Vectors and Icons](https://www.svgrepo.com/)

[The complete guide to building your personal self hosted server for streaming and ad-blocking powered by Plex, Jellyfin, Adguard Home and Docker. - Divisions by zero](https://lemmy.dbzer0.com/post/5911320)

[Things I Wish Someone Had Told Me When I Was Learning How to Code](https://www.freecodecamp.org/news/things-i-wish-someone-had-told-me-when-i-was-learning-how-to-code-565fc9dcb329/)

[Why Learning to Code is So Damn Hard | Thinkful](https://www.thinkful.com/blog/why-learning-to-code-is-so-damn-hard/)

[Have I Been Pwned: Check if your email has been compromised in a data breach](https://haveibeenpwned.com/)

[SauceNAO Reverse Image Search](https://saucenao.com/)

[Why Procrastinators Procrastinate — Wait But Why](https://waitbutwhy.com/2013/10/why-procrastinators-procrastinate.html)

[Don't ask to ask, just ask](https://dontasktoask.com/)

[Home - The XY Problem](https://xyproblem.info/)

[The Lesson to Unlearn](https://www.paulgraham.com/lesson.html)

[CallMeTee's Rush Bible, ver 1.10 - Google Docs](https://docs.google.com/document/d/1dz7O6hOPL9Usd8VbuCemHyjc3gSLYxaAGuHSVJukF1Q/edit)

[When to Use i | Medium](https://medium.com/@zac_heisey/when-to-use-strong-b-em-and-i-tags-in-your-markup-fa4d0af8affb)

[How to Write a Git Commit Message](https://cbea.ms/git-commit/)

[The Hitchhiker’s Guide to Online Anonymity | The Hitchhiker’s Guide to Online Anonymity](https://anonymousplanet.org/guide.html)

[codecrafters-io/build-your-own-x: Master programming by recreating your favorite technologies from scratch.](https://github.com/codecrafters-io/build-your-own-x)

[practical-tutorials/project-based-learning: Curated list of project-based tutorials](https://github.com/practical-tutorials/project-based-learning)

[DaFont - Download fonts](https://www.dafont.com/)

[Before starting - Hello Algo](https://www.hello-algo.com/en/chapter_hello_algo/)

[WinterBreak | Kindle Modding Wiki](https://kindlemodding.org/jailbreaking/WinterBreak/)

[KOReader](https://koreader.rocks/)

[index :: Git Cheatsheet :: NDP Software](https://ndpsoftware.com/git-cheatsheet.html#loc=index;)

[Basics | Cheat sheets | Linux Command Library](https://linuxcommandlibrary.com/)

[Free For Life](https://free.hrsn.dev/#/)

[Useful Tools 🔎 | Useful Tools](https://trolologuy.github.io/useful-tools/#vale)

[Enable or Disable the Refreshed New Tab Page Layout in Firefox – AskVG](https://www.askvg.com/enable-and-test-the-upcoming-new-tab-page-designs-in-firefox-right-now/)

[Enable or Disable New UI for New Tab Page Pinned Shortcuts in Firefox – AskVG](https://www.askvg.com/enable-or-disable-new-ui-for-new-tab-page-pinned-shortcuts-in-firefox/)

[Pandoc - index](https://pandoc.org/)

[FUTO Self Managed Guide](https://wiki.futo.org/index.php/Introduction_to_a_Self_Managed_Life:_a_13_hour_%26_28_minute_presentation_by_FUTO_software)
